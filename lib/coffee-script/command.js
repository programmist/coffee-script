...
(function() {
  var BANNER, CoffeeScript, EventEmitter, SWITCHES, compileJoin, compileOptions, compilePath, compileScript, compileStdio, exec, exists, forkNode, fs, helpers, hidden, insertImports, joinTimeout, links, lint, loadRequires, notSources, optionParser, optparse, opts, outputPath, parseOptions, path, printLine, printTokens, printWarn, removeSource, setDependants, sourceCode, sources, spawn, timeLog, unwatchDir, usage, version, wait, watch, watchDir, watchers, writeJs, _ref;
  ...
  insertImports = function(file, code, imports) {
    var dir, imp, importFile, insertCode, _i, _len;
  ...
  setDependants = function(file, code, imports) {
    var arr, dir, imp, importFile, _i, _len, _results;
    dir = path.dirname(file);
    _results = [];
    for (_i = 0, _len = imports.length; _i < _len; _i++) {
      imp = imports[_i];
      ...
      if (arr && arr.indexOf(file < 0)) {
        _results.push(arr.push(file));
      } else {
        _results.push(links[importFile] = [file]);
      }
    }
    return _results;
  };
  ...
  watch = function(source, base) {
    var compile, compileDependentFile, compileTimeout, prevStats, rewatch, watchErr, watcher;
    prevStats = null;
    compileTimeout = null;
    watchErr = function(e) {
      if (e.code === 'ENOENT') {
    ...
    compileDependentFile = function(depFile) {
      var depCode, depImports;
      ...
      return compileScript(depFile, depCode, path.dirname(depImports));
    };
    compile = function() {
      ...
      return compileTimeout = wait(25, function() {
        return fs.stat(source, function(err, stats) {
          ...
          prevStats = stats;
          return fs.readFile(source, 'utf-8', function(err, code) {
            var depFile, dependentFiles, imports, _i, _len;
            if (err) {
              return watchErr(err);
            }
            dependentFiles = links[source];
            if (dependentFiles) {
              for (_i = 0, _len = dependentFiles.length; _i < _len; _i++) {
                depFile = dependentFiles[_i];
